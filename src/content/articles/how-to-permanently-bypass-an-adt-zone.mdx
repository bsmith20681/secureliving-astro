...
<figure>
  <Image src={adtZonesScreen} alt="ADT Command panel zones screen showing sensor list and bypass options" />
  <figcaption>The Zones page on the ADT Command panel where you can bypass individual sensors</figcaption>
</figure>

<Admonition>
Something that can be misleading: if you enter the wrong PIN, the panel won't display an error code or give you any indication that something went wrong. Nothing happens. No message, no beep. The only way to tell your PIN didn't work is that the Do Not Enter icon won't show up. If everything looks the same after tapping Bypass, try entering your PIN again.
</Admonition>
...
- A sensor keeps causing false alarms every time you arm
...
If you're bypassing the same zone every single day just to avoid unnecessary alerts, the sensor probably needs to be replaced or removed from the system entirely. Call ADT security support at <a href={`tel:${companyData.phoneUnformatted}`}>{companyData.phone}</a> and they can troubleshoot the issue, check for problems on their end, or send a tech out to replace it. Have your panel's model number ready before you call so they can pull up your system info.
